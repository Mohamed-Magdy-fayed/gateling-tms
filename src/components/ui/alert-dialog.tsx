  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Popup
        ...
        data-size={size}
        className={cn(
          // Alert dialogs are always header + footer (no body slot), so the
          // flex column pins the footer and lets the header absorb the
          // overflow — AlertDialogDescription is the part that scrolls.
          "group/alert-dialog-content fixed top-1/2 start-1/2 z-50 flex max-h-[calc(100dvh-2rem)] w-full -translate-x-1/2 rtl:translate-x-1/2 -translate-y-1/2 flex-col gap-3 overflow-hidden rounded-xl bg-popover p-4 text-popover-foreground ring-1 ring-foreground/10 duration-100 outline-none data-[size=default]:max-w-xs data-[size=sm]:max-w-64 data-[size=default]:sm:max-w-sm data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95",
          className,
        )}
        {...props}
      />
...
function AlertDialogDescription({
  ...
  return (
    <AlertDialogPrimitive.Description
      ...
      className={cn(
        "min-h-0 overflow-y-auto text-xs/relaxed text-balance text-muted-foreground md:text-pretty *:[a]:underline *:[a]:underline-offset-3 *:[a]:hover:text-foreground",
        className,
      )}
      {...props}
    />
