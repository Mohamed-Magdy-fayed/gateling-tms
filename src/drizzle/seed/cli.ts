...
const commands = {
  ...
  demo: {
    description:
      "Seed the realistic-academy screenshot/demo dataset on top of baseline: 2 courses w/ levels+lectures+quiz, 3 groups w/ schedules+sessions (one seeded as already started), 25 students w/ enrollments/attendance/certificates.",
    action: async () => {
      const { runSeedProfile } = await import("@/drizzle/seed");
