...
- Make HTTP requests to external services (Google Forms API calls, webhooks)
...
2. **Reliability** — Inngest retries failed jobs automatically. A transient SMTP or third-party API error does not break the mutation.
...
| 6 | *(none)* — live-class meetings are created **inline** on demand while the teacher waits for the link, which is the documented exception to the rule above (blueprint D143). onMeeting has no webhooks, so there is no inbound sync to offload either. |
