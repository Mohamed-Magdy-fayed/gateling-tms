...
|---|---|---|---|---|
| Landing pages | `src\app\(landing-pages)\` — page sections `_components\{hero,value-proposition,features-preview,process,final-cta}-section*`, `features\`, `pricing\{page.tsx,data.ts}`, `about\ contact\ privacy\ terms\ refund\ cookies\`, `_layout\`, `_shared\` | `app\(landing)\` + `features\` translations | **adapt** | Rewrite copy per `00-product-spec.md`: truthful free tier, "powered by onMeeting", paid = coming soon, Gateling-TMS branding, drop fake trust stats ("50+ academies"). Keep `pricing\data.ts` PLAN_CONFIGS values. Translations re-keyed to B i18n engine (same `dt` API — mostly mechanical). |
...
| onMeeting | `src\lib\zoom\onmeeting.ts` (the API client), `src\server\api\routers\learning-flow\zoom-clients-router.ts` (`createOnMeetingKeys`), `src\components\modules\online-lectures\zoom-clients\zoom-clients-onmeeting-form.tsx` | `features\system\live-classes\` + `integrations\onmeeting\` | **adapt** | Org-scoped accounts, one row per room; sessions linked to groups; attendance into `session_students`, teacher-marked. Responses get Zod-validated rather than cast. **Do not port** `getMeetingDetails`'s HTML scraping or the embedded Zoom Web SDK page — both served an in-app meeting view this rebuild doesn't have. SOURCE's direct-Zoom code (`meeting-helpers.ts`, `app\api\webhooks\zoom\route.ts`, inngest `functions\zoom-clients.ts`) is **not a donor any more** — see D142. |
