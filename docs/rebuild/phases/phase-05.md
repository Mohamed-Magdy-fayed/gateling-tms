...
3. **Groups (classes).** CRUD with weekly schedule (day/time slots — SOURCE `groups-table.ts` schedule shape; UI with react-day-picker where helpful). **Creatable with no course attached.** Assign/remove students (multi-select from trainees; creating a new student inline from the group screen must work — the "add students directly on the spot" promise). On create/schedule-change → fire `group/schedule-changed` (one event covers both, STATE.md D80 — was written here as `group/created`) → Inngest generates sessions (adapt SOURCE `group-created.ts`; C `src\lib\slots.ts` as slot-math reference). Sessions land in `sessions` (renamed from `zoom_sessions`, D80) as unlinked/offline sessions; Phase 6 attaches an onMeeting meeting to one only when its class is actually started (D143).
