...
6. **Ops readiness.** Production env checklist in TARGET `docs\deploy.md` (env matrix per `06-workflow.md` §5): **Neon** main branch (prod) + preview branching healthy, Vercel Production scope complete (Upstash, SMTP, Firebase, Inngest keys, `ONMEETING_CREDENTIALS_ENCRYPTION_KEY`, Google OAuth prod origins), `db:migrate` verified in the deploy step against Neon, Neon backup/restore note. Confirm preview→prod parity: the last merged PR's preview behaved identically to prod post-deploy.
