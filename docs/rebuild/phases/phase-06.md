...
3. **Session ↔ meeting linking.** Phase-5-generated `sessions` (renamed from `zoom_sessions`, STATE.md D80 — this phase adds the Zoom columns to it) get real Zoom meetings: on session create/update (Inngest) call Zoom API via the org's client → store meeting id/join/start URLs. Sessions without a connected client stay "offline" gracefully — Zoom is optional, not a blocker (a free org without Zoom still schedules classes).
